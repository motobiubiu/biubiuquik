...
__global__ void reluKernel(float* A,float* output,int N) {
    // int row = blockIdx.y * blockDim.y + threadIdx.y;
    int idx= blockIdx.x * blockDim.x + threadIdx.x;

    if (idx < N) {
        output[idx]=A[idx]>0?A[idx]:0;
    }
}

void reluCUDA(float* h_A, float* h_B, int N) {
    // 分配设备内存
    float *d_A, *d_B;
    ...
    // 启动核函数
    reluKernel<<<blocksPerGrid, threadsPerBlock>>>(d_A, d_B,N);
...
int main() {
    ...
    auto time2=measureExecutionTime(reluCUDA,input.data(), output2.data(),n);
