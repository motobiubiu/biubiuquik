...
__global__ void sigmoidKernel(float* A,float* output,int N) {
    // int row = blockIdx.y * blockDim.y + threadIdx.y;
    int idx= blockIdx.x * blockDim.x + threadIdx.x;

    if (idx < N) {
        output[idx]=1.0f/(1+__expf(-A[idx]));
        
    }
}

void sigmoidCUDA(float* h_A, float* h_B, int N) {
    // 分配设备内存
    float *d_A, *d_B;
    ...
    // 启动核函数
    sigmoidKernel<<<blocksPerGrid, threadsPerBlock>>>(d_A, d_B,N);
...
int main() {
    ...
    auto time2=measureExecutionTime(sigmoidCUDA,input.data(), output2.data(),n);
