...
def benchmark(size, provider):
    ...
    if provider == 'torch':
        ms, min_ms, max_ms = triton.testing.do_bench(lambda:torch.where(x>0,x,alpha*(torch.exp(x)+1)), quantiles=quantiles)
    if provider == 'triton':
        ms, min_ms, max_ms = triton.testing.do_bench(lambda: elu(x,alpha), quantiles=quantiles)
