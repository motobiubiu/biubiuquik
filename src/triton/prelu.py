...
def benchmark(size, provider):
    x = torch.rand(size, device='cuda', dtype=torch.float32)*2-1
    alpha = torch.rand(1, device='cuda', dtype=torch.float32)
    quantiles = [0.5, 0.2, 0.8]
    if provider == 'torch':
        ms, min_ms, max_ms = triton.testing.do_bench(lambda: torch.where(x>0,x,alpha*x), quantiles=quantiles)
    if provider == 'triton':
        ms, min_ms, max_ms = triton.testing.do_bench(lambda: prelu(x,alpha), quantiles=quantiles)
