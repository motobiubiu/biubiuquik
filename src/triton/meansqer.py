...
def benchmark(size, provider):
    ...
    if provider == 'torch':
        ms, min_ms, max_ms = triton.testing.do_bench(lambda: torch.sum((x-y)), quantiles=quantiles)
    if provider == 'triton':
        ms, min_ms, max_ms = triton.testing.do_bench(lambda: meansqer(x, y), quantiles=quantiles)
