...
private:
    ...
    __aicore__ inline void Compute(int32_t progress)
    {
        ...
        half maxnum = 0;
        AscendC::Duplicate<half>(tmp1, maxnum, TILE_LENGTH);
        AscendC::Max(xLocal, xLocal, tmp1, TILE_LENGTH);
        half minnum = 6;
        AscendC::Duplicate<half>(tmp2, minnum, TILE_LENGTH);
        AscendC::Min(zLocal, xLocal, tmp2, TILE_LENGTH);  
...
extern "C" __global__ __aicore__ void relu6_custom(GM_ADDR x, GM_ADDR z)
{
    KernelRelu6 op;
    op.Init(x, z);
    op.Process();
